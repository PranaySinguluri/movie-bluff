...
//         <button
//           onClick={scrollToTop}
//           className="fixed bottom-40 right-4 bg-blue-500 text-white rounded-full p-4 shadow-lg cursor-pointer hover:bg-blue-600 transition-colors"
...
function Homly({
  ...
  onLogout = () => {},
  defaultSearchQuery = "",
  theme = "bg-gradient-to-r from-blue-500 to-purple-600",
}) {
  const [url, setUrl] = useState("");
  ...
  return (
    <div className={`min-h-screen ${theme}`}>
      <NavBar />
      <h1
        className="text-white text-4xl p-4 cursor-pointer"
      ...
      <div className="wrapper-search px-4">
        ...
        {movies?.length > 0 && (
          <div className="movie-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-4">
            {movies.map((movie) => (
              <div
                className="movie-card bg-gray-800 text-white rounded-lg shadow-lg p-4 hover:scale-105 transition-transform"
                key={movie.id}
              >
                <h2 className="text-lg font-semibold truncate">{movie.title}</h2>
                <p className="text-sm">{movie.release_date?.split("-")[0]}</p>
